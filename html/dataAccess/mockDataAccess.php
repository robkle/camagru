...
class MockDataAccess implements DataAccess
{
	...
	{	
		$db_user = [];
		if (($handle = fopen("mocks/mockDatabase/users.csv", "r")) !== FALSE)
		{
	...
	{
		$SUCCESS = false;
		if (($handle = fopen("mocks/mockDatabase/users.csv", "a")) !== FALSE)
		{
	...
	public function confirmUser($ckey): bool 
	{
		...
		if (($handle = fopen("mocks/mockDatabase/users.csv", "r")) !== FALSE)
		{
			if (($handle_tmp = fopen("mocks/mockDatabase/.tmpusers.csv", "a")) !== FALSE)
			{
				while (($line = fgetcsv($handle, null, ",")) !== FALSE)
				{
					if ($line[5] == $ckey) {
		...
		fclose($handle);
		rename("mocks/mockDatabase/.tmpusers.csv", "mocks/mockDatabase/users.csv");
		return $SUCCESS;
	}
}
